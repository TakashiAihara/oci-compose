...
oi1 上で 1 分間隔の cron で動き、 `aihara.online` apex の DNS A レコード (= 自宅 Starlink global IP) を OCI NSG の ingress rule (443/tcp の source CIDR) に反映する。 ペア側の DDNS 更新は pi 上の `cloudflare-ddns` (favonia/cloudflare-ddns) が担当。
...
1 min cron
  └─ sync.sh
       ...
       ├─ oci network nsg-security-rule list で managed rule (protocol=6, port=443) を特定
       └─ oci network nsg-security-rule update で source を <new IP>/32 に上書き
```
...
| `INGRESS_PORT` | いいえ | `443` | 対象ポート (rule 特定にも使用) |
| `INGRESS_PROTOCOL` | いいえ | `6` (TCP) | 対象プロトコル番号 |
...
- `no managed ingress rule found`: NSG に初期 dummy rule (port 443/tcp) が無い、 OCI コンソールで作成する
