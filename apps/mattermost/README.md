...
| 公開 host | `mattermost.takashiaihara.site` |
...
- Domain: `mattermost.takashiaihara.site` (service `mattermost` にバインド、 wildcard cert 既存)
...
deploy 済み・HTTPS `mattermost.takashiaihara.site/api/v4/system/ping` = 200 (status OK) を確認済み。

## 初回セットアップ (ユーザー作業)

1. `https://mattermost.takashiaihara.site` を開いて初回管理者アカウントを作成 (自宅 IP からのみ到達可、 NSG home-only 傘下)
...
4. そのチャンネルで Incoming Webhook を発行し、 URL を Infisical `home/prod` に `MATTERMOST_9ROUTER_WEBHOOK_URL` 等で格納
   - webhook URL は `https://mattermost.takashiaihara.site/hooks/xxxx` 形式
